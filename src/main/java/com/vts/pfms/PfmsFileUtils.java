...
@Component 
public class PfmsFileUtils 
{

	public void addWatermarktoPdf(String pdffilepath,String newfilepath,String waterMarkName) throws Exception
	{
		...
		try (PdfDocument doc = new PdfDocument(new PdfReader(pdffile), new PdfWriter(tofile))) {
			...
			for (int pageNum = 1; pageNum <= doc.getNumberOfPages(); pageNum++) {
				...
				try (Canvas canvas2 = new Canvas(canvas,  page.getPageSize())) {
					...
					Paragraph watermark = new Paragraph(waterMarkName.toUpperCase())
							.setFont(helvetica)
							.setFontSize(100f)
	...
	public File addWatermarktoPdf1(String pdffilepath,String newfilepath,String waterMarkName) throws Exception
	{
		File pdffile = new File(pdffilepath);
		File tofile = new File(newfilepath);
	...
	public void addWatermarktoPdf2(String pdffilepath,String newfilepath,String waterMarkName) throws Exception
	{
		File pdffile = new File(pdffilepath);
		File tofile = new File(newfilepath);
	...
	public void addWatermarktoCCMPdf(String pdffilepath, String newfilepath, String waterMarkName) throws Exception {
		File pdffile = new File(pdffilepath);
		File tofile = new File(newfilepath);
		...
		try (PdfDocument doc = new PdfDocument(new PdfReader(pdffile), new PdfWriter(tofile))) {
			...
			for (int pageNum = 1; pageNum <= doc.getNumberOfPages(); pageNum++) {
				PdfPage page = doc.getPage(pageNum);
				removeExistingWatermark(page, "ANNEX-");
				PdfCanvas canvas = new PdfCanvas(page.newContentStreamBefore(), page.getResources(), doc);
	...
	private void removeExistingWatermark(PdfPage page, String prefix) {
        ...
        if (contentArray != null) {
            for (int i = 0; i < contentArray.size(); i++) {
                ...
                // Regex pattern to match watermark text starting with the prefix and followed by numbers
                String regex = prefix + "\\d+(-\\d+)?";
                content = content.replaceAll(regex, ""); // Remove watermark matching the pattern
