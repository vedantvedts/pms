										    ...
										    <form action="#" method="POST" name="myfrm" style="display: inline">
										        ...
										        <%if(obj[5]!=null && iddforwardstatus.contains(obj[5].toString()) ) {%>
													...
													<%if(obj[5]!=null && "RFW".equalsIgnoreCase(obj[5].toString()) ) {%>
														<button class="editable-clicko" name="revoke" formaction="IDDDocumentUserRevoke.htm" formmethod="post" onclick="return confirm('Are you sure to Revoke?')">
															<div class="cc-rockmenu">
																<div class="rolling">
																	<figure class="rolling_icon">
	           			...
	           			<table align="center"  >
	        				<tr>
	        					<td class="trup" style="background: linear-gradient(to top, #3c96f7 10%, transparent 115%);">
	         						Prepared By - <%if(iddApproval!=null) {%><%=StringEscapeUtils.escapeHtml4(iddApproval[10].toString()) %> <%} else{%>Prepared By<%} %>
	         					</td>
	        					...
	        					<td class="trup" style="background: linear-gradient(to top, #eb76c3 10%, transparent 115%);">
	        						Reviewer - <%if(iddApproval!=null) {%><%=StringEscapeUtils.escapeHtml4(iddApproval[9].toString()) %> <%} else{%>Reviewer<%} %>
	        	    			</td>
	                    		...
	                    		<td rowspan="2">
	             				...
	             				<td class="trup" style="background: linear-gradient(to top, #9b999a 10%, transparent 115%);">
	             					Approver - <%if(iddApproval!=null) {%><%=StringEscapeUtils.escapeHtml4(iddApproval[8].toString()) %> <%} else{%>Approver<%} %>
	             	    		</td>
	            			</tr> 	
	            	    </table>			             
					</div>
