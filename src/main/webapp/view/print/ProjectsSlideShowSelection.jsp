...
<!-- Modal -->
	    ...
	    <div class="modal-content">
		        		...
		        		<div class="col align-items-center" >
			        		<div class="row align-items-center">
				        		<div class="col" style="width: 75%;" >
						        	<select id="fav" class="form-control" onclick="isAddFavClicked('select')">
	      ...
	      <div class="row" style="width: 100%">
	      	...
	      	<div class="col d-flex justify-content-end" >
	        	...
	        	<button type="submit"  class="btn btn-primary mx-1">Slide Show</button>
	       </div>
	      </div>
	...
	<div class="modal fade" id="selectFavProjectList" style="width: 100%"
		...
		aria-hidden="true">
		<form action="saveFavSlides.htm" method="post" id="SavingFav"
			onsubmit="return checkFavSlideSub()">
			...
			<div class="modal-dialog modal-lg" style="max-width: 1800px"
				...
				<div class="modal-content">
					<div class="modal-header">
						...
						<button type="button" class="close" onclick="$('#selectFavProjectList').modal('hide')"
							aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary"
							onclick="$('#favTitle').val('');$('#selectFavProjectList').modal('hide');isAddFavClicked('select') ">Close</button>
...
$(document).ready(function(){
    ...
    $("#selectallproject").click(function(){
        var isChecked = $(this).prop('checked');
        $('.mainprojectlis, .subprojectlis, #selectmainproject, #selectsubproject').prop('checked', isChecked);
    });
    ...
    $('.mainprojectlis').click(function() {
        if (!$(this).prop('checked')) {
            $('#selectmainproject').prop('checked', false);
            $('#selectallproject').prop('checked', false);
        } 
        else {
           var allMainChecked = true;
           ...
           if (allMainChecked) {
               ...
               if ($('#selectsubproject').prop('checked')) {
                   $('#selectallproject').prop('checked', true);
               }
           }
        }
    ...
    // Handling click event for individual checkboxes inside Sub Projects
    $('.subprojectlis').click(function() {
        if (!$(this).prop('checked')) {
            $('#selectsubproject').prop('checked', false);
            $('#selectallproject').prop('checked', false);
        } else {
            var allSubChecked = true;
            ...
            if (allSubChecked && $('#selectmainprojec').prop('checked')) {
                $('#selectallproject').prop('checked', true);
            }
        }
    });
...
function isAddFavClicked(tag)
{
	...
	else if(val != "defaultValue")
	{
		//write code for selecting the checkboxes in slideshow checkbox projects with respect to selected fav
		 ...
		 if(tag != 'select'){document.getElementById("favTitle").value=options[Fav.selectedIndex-2][1];
