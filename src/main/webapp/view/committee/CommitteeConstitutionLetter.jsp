...
%>
 ...
 <div  >
 <span style="float: left; font-size:13px;">Ref No. - <%if(committeemaindata[11]!=null) {%><%= projectCode.length()>1?projectCode+"/ ":"" %><%=StringEscapeUtils.escapeHtml4(committeemaindata[11].toString())%><%}else{ %> -<%} %> </span>
 <span style="float: right; font-size:13px;">Date :  <%if(committeemaindata[12]!=null){ %>  <%=sdf.format(sdf1.parse(StringEscapeUtils.escapeHtml4(committeemaindata[12].toString())))%><%} %></span>
 </div>  
<br>
 	<div style="text-align: center;" ><h3 style="margin-bottom: 2px;" align="center"><%=labdetails[2]!=null?StringEscapeUtils.escapeHtml4(labdetails[2].toString()): " - "+"("+labdetails[1]!=null?StringEscapeUtils.escapeHtml4(labdetails[1].toString()): " - "+")" %> </h3></div>  
 	
	<div style="text-align: center;" ><h3 style="margin-bottom: 2px;" align="center">Formation of Committee  <%=committeeedata[2]!=null?StringEscapeUtils.escapeHtml4(committeeedata[2].toString()): " - "%> (<%=committeeedata[1]!=null?StringEscapeUtils.escapeHtml4(committeeedata[1].toString()): " - "%>) </h3></div>
	<br>
	<div  align="center">
				...
				<div style="text-align: center;" >
					<div style="margin-bottom: 2px; max-width: 650px;text-align: justify;text-justify: inter-word;text-align: justify;text-justify: inter-word;" align="center">
						<%if(Long.parseLong(projectid)>0 || Long.parseLong(divisionid)>0 || Long.parseLong(initiationid)>0){ %>
								<%if(committeedescription[1]!=null){ %><%=StringEscapeUtils.escapeHtml4(committeeedata[1].toString()) %> <%}else{ %>No Data <%} %>
						<%}else { %>
								<%if(committeeedata[10]!=null){ %><%=StringEscapeUtils.escapeHtml4(committeeedata[10].toString())%> <%}else{ %>No Data <%} %>
						<%} %>
					</div>
				</div>
			</td>
...
<!-- 	<table style=" margin-top: 10px; margin-bottom: 10px; margin-left: 15px; width: 650px; font-size: 16px; border-collapse:collapse; " >
	...
	<table style=" margin-bottom: 10px; margin-left: 15px; width: 650px; font-size: 13px; border-collapse:collapse;border:1px solid black; " >
		...
		<% int i=0;
			...
			<tr>				
				...
				<td style="max-width:300px;text-align: left; padding: 5px 0px 5px 0px;border:1px solid black;">&nbsp;<%=member[2]!=null?StringEscapeUtils.escapeHtml4(member[2].toString()): " - " %><%=member[4]!=null &&  member[4].toString().length()>1?", "+StringEscapeUtils.escapeHtml4(member[2].toString()):"" %> <%-- <%if(member[8].toString().equals("CW")){ %><%=member[9]%><%}  %> --%>&nbsp;</td>
		...
		<tr>
			<td>
				<div style="text-align: center;" >
					...
					<%if(Long.parseLong(projectid)>0 || Long.parseLong(divisionid)>0 || Long.parseLong(initiationid)>0){ %>
						<%if(committeedescription[2]!=null){ %><%=StringEscapeUtils.escapeHtml4(committeedescription[2].toString())%> <%}else{ %>No Data <%} %>
														
					<%}else if(projectid!=null && Long.parseLong(projectid)==0){ %>
								<%if(committeeedata[11]!=null){ %><%=StringEscapeUtils.escapeHtml4(committeeedata[11].toString())%> <%}else{ %> No Data <%} %>
					<%} %>
	...
	<br>
	<br>
	<%=CommitteMainEnoteList!=null && CommitteMainEnoteList[14]!=null?StringEscapeUtils.escapeHtml4(CommitteMainEnoteList[14].toString()):"" %>
	<%} %>
	</div>
	<br><br>
	<div style="text-align: left;font-size: 13px;">
	Initiated By : <%if(CommitteMainEnoteList!=null && CommitteMainEnoteList[18]!=null ){ %> <%=StringEscapeUtils.escapeHtml4(CommitteMainEnoteList[18].toString()) %>, <%=CommitteMainEnoteList[19]!=null?StringEscapeUtils.escapeHtml4(CommitteMainEnoteList[19].toString()): " - " %>  <%}else{ %>  <%= constitutionapprovalflow.get(0)[0]!=null?StringEscapeUtils.escapeHtml4(constitutionapprovalflow.get(0)[0].toString()): " - "%>,  <%= constitutionapprovalflow.get(0)[1]!=null?StringEscapeUtils.escapeHtml4(constitutionapprovalflow.get(0)[1].toString()): " - "%> <%} %>
	</div>
