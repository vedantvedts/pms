	...
	<div class="row">
		<div class="col-md-12">	
			...
			<div class="card shadow-nohover">	
					...
					<div class="col-md-2">
					<input class="form-control" name="Approving_Role" maxlength="18" value="<%if(CommitteMainEnoteList!=null && CommitteMainEnoteList[14]!=null) {%><%=CommitteMainEnoteList[14].toString() %> <%}%>" required="required">
					</div>
					<div class="col-md-2">
					...
					<button type="button"  class="btn btn-sm submit" onclick="return checkOfficer('F')"   <%if(!forwardstatus.contains(CommitteMainEnoteList[15].toString())) {%>	disabled="disabled"<%} %> id="Forward">Forward</button>
					<%} %>
			...
			function checkOfficer(value){
				...
				if(Recommend3!==0 && Recommend2===0){
					alert("Recommended Officer2 can not be empty !")
					return false;
				}
				if(Recommend2!==0 && !Rec2_Role.length>0){
					alert("Recommended Officer2 role can not be empty!");
					 $('#Rec2_Role').focus();
					return false;
				}
				if(Recommend3!==0 && !Rec3_Role.length>0){
					alert("Recommended Officer3 role can not be empty!");
					 $('#Rec3_Role').focus();
					return false;
				}
				...
				for(var i=0;i<array.length-1;i++){
					if(array[i]!==0){
						for(var j=i+1;j<array.length;j++ ){
							if(array[i]===array[j]){
								alert("Two officers can not be Same.")
								return false;
							}
						}
					}
