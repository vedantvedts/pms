				...
				<div class="card shadow-nohover" >
					<div class="card-header">
					 ...
					 <form action="RODScheduleDelete.htm"  method="post" ><%} %>
	                        		...
	                        		<tr>
	                            		<td><label class="control-label">Meeting Date : &nbsp;</label></td>          						
							   			<td>
					   							...
					   							if(Integer.parseInt(rodscheduleeditdata[10].toString())<5)
					   							{ %>	
					   								<input  class="form-control"  data-date-format="dd/mm/yyyy" id="readonlystartdate" name="committeedate" readonly="readonly" required="required"   value=" <%= sdf.format(sdf1.parse( rodscheduleeditdata[2].toString()))%>">
					   							<%}else{ %>				   									
					   								<input  class="form-control"  data-date-format="dd/mm/yyyy" id="startdate" name="committeedate"  required="required" readonly="readonly"  value=" <%= sdf.format(sdf1.parse( rodscheduleeditdata[2].toString()))%>">
					   							<%} %>
							...
							<form action="RODVenueUpdate.htm" method="post" id="venuupdatefrm">
									<table style="margin-top: 10px; width: 95%" >
										<tr>
					                         ...
					                         <td >			                            
											    <select class="custom-select " id="isconfidential" required="required" name="isconfidential">
													...
													<% for (Object[] obj : pfmscategorylist) {%>
														<option value="<%=obj[0]%>" <% if(rodscheduleeditdata[13].toString().equals(obj[0].toString())) {%>selected <%} %>  ><%=obj[1]%></option>													
													<%} %>
											  </select>
					                        </td> --%>
		                  				</tr>
		                  				...
		                  				<tr>
		                  					<td colspan="4" >
						       					<textarea class="form-control" name="decisions" id="decisions" cols="50" rows="5"  placeholder="Decisions sought from Meeting" maxlength="1000"><%if(rodscheduleeditdata[18]!=null ){ %> <%=rodscheduleeditdata[18] %> <%} %></textarea>
						       				</td>
		                  				</tr> --%>
		                  				<tr >  
						...
						<hr>				
			         	...
			         	<table  class="table table-bordered table-hover table-striped table-condensed ">
			            	<thead>
			              		...
			              		<%	int count=0;
							for(Object[] 	obj:rodagendalist){ count++;%>  
						...
						<%--  <%if( rodscheduleeditdata[4].toString().equalsIgnoreCase("MAR") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MMR")  ){ %>
						<h5 style="color:#07689f">Reason For Returning</h5>
					   	<hr>
						<%for(Object[] obj : returndata){ %>
						
						<%if(rodscheduleeditdata[4].toString().equalsIgnoreCase(obj[2].toString())) {%>
			           ...
			           <%-- <%if(useraccess>=1){ %> --%>
							...
							<div><h5 style="color:#07689f"> Operations </h5>
								 ...
								 <%-- <%if( rodscheduleeditdata[4].toString().equalsIgnoreCase("MMF") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MMA") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MMR")  ){ %>
								 
									 <form method="POST" action="RODMinutesViewAllDownload.htm" name="myfrm" id="myfrm" style="float: right"> 
									 
									 	<%if(rodscheduleeditdata[20]!=null){ %>
									 	<a  class="btn  btn-sm view" href="MinutesAttachDownload.htm?attachmentid=<%=rodscheduleeditdata[20] %>" target="_blank" style="background-color:#009DAE ;color:white ;font-size:12px;" >ATTACHED MINUTES </a>
									 	<%} %>
									 	<%if(Long.parseLong(projectid)>0){ %>
							...
							<%-- <%}else{ %>
								<div class="row">
									...
									<h5> <span style="margin-left: 30px; font-size: 20px">Note : &nbsp;</span><span style="color: red"><%=rodscheduleeditdata[7] %>  <%if(Integer.parseInt(projectid)>0){ %>							  
							...
							<%} %> --%>
								...
								<div class="form-block">
									...
									<form action="RODScheduleMinutes.htm" name="myfrm" id="myfrm" method="POST">
										
										
														<%-- <%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MAA") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MKO") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MKV") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MMF") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MMA") || rodscheduleeditdata[4].toString().equalsIgnoreCase("MMR") ){ %>
														...
														<%-- <%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MMA")){ %>
														...
														if(Integer.parseInt(rodscheduleeditdata[10].toString())<3 && invitedlist.size()>0)
															{%>
																<input type="submit" id="submit" class="btn  btn-sm back" value="Participants" formaction="RODInvitations.htm" style="background-color:#0e49b5 	;color:white" />
															<%}
															else if(invitedlist.size()>0 && Integer.parseInt(rodscheduleeditdata[10].toString())<8 ){ %>
																<input type="submit" id="submit" class="btn  btn-sm submit" value="Participants" formaction="RODInvitations.htm" style="background-color:#0e49b5 	;color:white;border-color:#0e49b5 " />
															<%}else if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MSC") ) { %>
																<input type="submit" id="submit" class="btn  btn-sm submit" value="Invite" onclick="invite()" formaction="RODInvitations.htm" style="background-color:#0e49b5 	;color:white" />
														...
														<%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MKV") ){ %>
															...
															<input type="submit" id="submit" class="btn  btn-sm submit" value="Attendance" formaction="RODAttendance.htm" style="background-color:#16697a ;color:white;border-color:#16697a " />
														...
														<%-- <%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MMR")){ %>
									...
									<%if(rodagendalist.size()>0 && invitedlist.size()>0 ){ %>
											...
											<%-- <%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MSC") ) { %>
											...
											<%} %>
											<%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MAF")){ %>
											...
											<%} %> 
											<%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MAR")){ %>
											...
											<%} %> --%>
											<%-- <%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MMF")){ %>
									...
									<form action="KickOffRODMeeting.htm" name="myfrm" id="myfrm" method="post">
			
										<%if(rodscheduleeditdata[4].toString().equalsIgnoreCase("MSC") && !todaydate.isBefore(scheduledate) && invitedlist.size()>0 ){ %>
										...
										<%-- <%if( rodscheduleeditdata[4].toString().equalsIgnoreCase("MKO") ){ %>
										...
										<input type="hidden" name="${_csrf.parameterName}"	value="${_csrf.token}" /> 
										<input type="hidden" name="committeescheduleid" value="<%=rodscheduleeditdata[6] %>">	
										<input type="hidden" name="rodname" value="<%=rodscheduleeditdata[7] %>">
										<input type="hidden" name="meetingdate" value="<%=rodscheduleeditdata[2] %>">
										<input type="hidden" name="meetingtime" value="<%=rodscheduleeditdata[3] %>">
										<input type="hidden" name="meetingid" value="<%=rodscheduleeditdata[11] %>">
										<input type="hidden" name="projectid" value="<%=projectid %>">
										<input type="hidden" name="rodNameId" value="<%=rodscheduleeditdata[0]%>">
								...
								<form  action="#" method="post" id="myfrm" target="_blank">
									<%if(Integer.parseInt(rodscheduleeditdata[10].toString())>=6) { %>
										...
										<input type="hidden" name="${_csrf.parameterName}"	value="${_csrf.token}" />
									<%} %>
									<% if(rodscheduleeditdata[23].toString().equalsIgnoreCase("Y")){%>
										<button type="submit" class="btn btn-sm back" formaction="MeetingBriefingPaper.htm" name="scheduleid" value="<%=rodscheduleeditdata[6] %>" formmethod="get" data-toggle="tooltip" data-placement="bottom" title="Briefing Paper" >
											BRIEFING
										</button>
									<%}%>
									<%} %> 
